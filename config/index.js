...
const config = {
  ...
  state: {
    ...
    stateFile: require('path').join(environment.STATE_DIR, 'audio_router_state.json'),
    backupFile: require('path').join(environment.STATE_DIR, 'audio_router_state.backup.json')
  },
...
config.validate = function() {
  ...
  // Validate JACK tools path in Windows environments
  if (process.platform === 'win32' || process.env.WSL_DISTRO_NAME) {
    const jackLspPath = path.join(this.JACK_TOOLS_PATH, 'jack_lsp.exe');
    if (!fs.existsSync(jackLspPath)) {
      errors.push(`JACK tools not found at ${jackLspPath}. Please install JACK2 or update JACK_TOOLS_PATH`);
    }
  }
  
  // Validate port conflicts
  if (this.PORT === this.HTTPS_PORT) {
    errors.push(`Port conflict: HTTP and HTTPS ports cannot be the same (${this.PORT})`);
  }
  
  // Validate log level
  const validLogLevels = ['ERROR', 'WARN', 'INFO', 'DEBUG'];
  if (!validLogLevels.includes(this.LOG_LEVEL)) {
    errors.push(`Invalid log level: ${this.LOG_LEVEL}. Valid levels: ${validLogLevels.join(', ')}`);
  }
...
config.printSummary = function() {
  ...
  console.log(`HTTPS Server: ${this.HOST}:${this.HTTPS_PORT} ${this.server.https.enabled ? '(enabled)' : '(disabled)'}`);
  ...
  console.log(`Features: ${Object.entries(this.FEATURES).filter(([_, enabled]) => enabled).map(([name]) => name).join(', ')}`);
  console.log('─'.repeat(50));
...
if (validationErrors.length > 0 && !config.isTest) {
  ...
  if (validationErrors.some(error => error.includes('JACK tools not found'))) {
    console.log('\n💡 To fix JACK tools issue:');
    console.log('   1. Install JACK2 for Windows from https://jackaudio.org/downloads/');
    console.log('   2. Or set JACK_TOOLS_PATH environment variable to correct path');
