...
if (isNaN(HTTPS_PORT) || HTTPS_PORT < 1 || HTTPS_PORT > 65535) {
  throw new Error(
    `Invalid HTTPS_PORT: ${process.env.HTTPS_PORT}. Must be a number between 1 and 65535.`
  );
}
...
if (!validLogLevels.includes(LOG_LEVEL)) {
  console.warn(
    `Invalid LOG_LEVEL: ${LOG_LEVEL}. Using INFO. Valid levels: ${validLogLevels.join(', ')}`
  );
}

// JACK configuration
const JACK_TIMEOUT = parseInt(process.env.JACK_TIMEOUT || '10000', 10);
const JACK_STATUS_CACHE_TTL = parseInt(
  process.env.JACK_STATUS_CACHE_TTL || '5000',
  10
);
...
const STATE_AUTO_SAVE_INTERVAL = parseInt(
...
const HEALTH_CHECK_CACHE_TTL = parseInt(
  process.env.HEALTH_CHECK_CACHE_TTL || '10000',
  10
);

// Development configuration
const DEV_PROXY_TARGET =
  process.env.DEV_PROXY_TARGET || 'http://localhost:5173';
...
const MQTT_CLIENT_ID =
  process.env.MQTT_CLIENT_ID || `jack-router-${Date.now()}`;
const MQTT_KEEPALIVE = parseInt(process.env.MQTT_KEEPALIVE || '60', 10);
const MQTT_RECONNECT_PERIOD = parseInt(
  process.env.MQTT_RECONNECT_PERIOD || '1000',
  10
);

// Device configuration for MQTT
const DEVICE_MODEL = process.env.DEVICE_MODEL || 'JACK Audio Router';
const DEVICE_MANUFACTURER = process.env.DEVICE_MANUFACTURER || 'Custom';
...
const validatedConfig = {
  ...
  // JACK
  JACK_TIMEOUT: validateTimeout(
    process.env.JACK_TIMEOUT,
    'JACK_TIMEOUT',
    '10000'
  ),
  JACK_STATUS_CACHE_TTL: validateTimeout(
    process.env.JACK_STATUS_CACHE_TTL,
    'JACK_STATUS_CACHE_TTL',
    '5000'
  ),
  JACK_TOOLS_PATH,

  // State
  STATE_AUTO_SAVE_INTERVAL: validateTimeout(
    process.env.STATE_AUTO_SAVE_INTERVAL,
  ...
  // Paths (make sure ssl is properly structured)
  PROJECT_ROOT,
  SSL_DIR,
  STATE_DIR,
  DIST_DIR,
  ...
  // Health
  HEALTH_CHECK_TIMEOUT: validateTimeout(
    process.env.HEALTH_CHECK_TIMEOUT,
    'HEALTH_CHECK_TIMEOUT',
    '5000'
  ),
  HEALTH_CHECK_CACHE_TTL: validateTimeout(
    process.env.HEALTH_CHECK_CACHE_TTL,
